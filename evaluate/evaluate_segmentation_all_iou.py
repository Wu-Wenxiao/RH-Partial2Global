...
def evaluate(args):
    with open(os.path.join(args.output_dir, 'log_all.txt'), 'w') as log:
        log.write(str(args) + '\n')
    padding = 1
    ...
    # import pdb;pdb.set_trace()
    ds = {
        'pascal': pascal_dataloader_all.DatasetPASCAL,
    ...
    length_ds = len(ds)
    for idx in trange(0, len(ds)):
        print(idx)
        # canvas_stack = ds[idx]['grid_stack']
        # breakpoint()
        save_name = str(idx)+".pt"
        save_path = os.path.join(args.output_dir, save_name)
        canvas_stack = torch.load(save_path)
        # canvas_stack = ds[idx]['grid_stack']
        lines = []
        for grid_idx, canvas in enumerate(tqdm(canvas_stack)):
            # breakpoint()
            # print(grid_idx)
            ...
            lines.append(str(idx) + '\t' + str(grid_idx) + '\t' + str(current_metric) + '\n')
            
            for i, j in current_metric.items():
                eval_dict[i] += (j / len(ds))

        with open(os.path.join(args.output_dir, 'log_all.txt'), 'a') as log:
