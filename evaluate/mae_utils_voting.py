...
def generate_mask_for_evaluation(voting=False):
    mask = create_maek(voting)
    if not voting:
        mask = obtain_values_from_mask(mask)
        len_keep = len(mask)
        return fill_to_full(mask), len_keep
    else:
        mask_list, len_keep_list = [], []
        for i in range(mask.shape[0]):
            mask_ = obtain_values_from_mask(mask[i])
            len_keep_list.append(len(mask_))
            mask_list.append(fill_to_full(mask_))
        return mask_list, len_keep_list
