...
def evaluate(args):
    with open(os.path.join(args.output_dir, 'log.txt'), 'w') as log:
        log.write(str(args) + '\n')
    with open(os.path.join(args.output_dir, 'log.csv'), 'w') as log:
        writer = csv.writer(log)
        # writer.writerow(["query_name","support_name","iou_score","To Remove"])
        writer.writerow(["query_name","support_name","iou_score"])
    padding = 1
    ...
    # import pdb;pdb.set_trace()
    ds = {
        'pascal': pascal_dataloader_csv.DatasetPASCAL,
    ...
    for idx in trange(len(ds)):
        ...
        for grid_idx, canvas in enumerate(canvas_stack):
            ...
            with open(os.path.join(args.output_dir, 'log.csv'), 'a') as log:
                writer = csv.writer(log)
                # writer.writerow([query_name,support_name[grid_idx],current_metric["iou"],flag])
                writer.writerow([query_name,support_name[grid_idx],current_metric["iou"]])
        # import pdb;pdb.set_trace()

    with open(os.path.join(args.output_dir, 'log.txt'), 'a') as log:
        log.write('all\t' + str(eval_dict) + '\n')
        print('all\t' + str(eval_dict) + '\n')
