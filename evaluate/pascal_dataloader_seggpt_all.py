...
class DatasetPASCAL(Dataset):
    ...
    def get_top50_images_val(self):
        ...
        path = self.annotaion_file # with open(f"./dataset/VOC2012/{self.feature_name}/folder{self.fold}_all-similarity.json") as f:
        print(path)
        with open(path) as f:
    ...
    def save_mask(self, img, name, mode):
        ...
        # os.makedirs(f'./temp_mask_{gpu_id}/'+ mode, exist_ok=True)
        os.makedirs(f'./temp_mask_0/folder_{self.fold}/gpu_{gpu_id}/'+ mode, exist_ok=True)
        # output_name = f'./temp_mask_{gpu_id}/'+ mode + "/" + name + ".png"
        output_name = f'./temp_mask_0/folder_{self.fold}/gpu_{gpu_id}/'+ mode + "/" + name + ".png"
    ...
    def sample_episode(self, idx, sim_idx):
        ...
        if support_name == query_name and sim_idx < len(self.images_top50_val[query_name]['top50'])-1:
