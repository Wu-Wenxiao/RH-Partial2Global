...
class DatasetPASCAL(Dataset):
    ...
    def save_mask(self, img, name, mode):
        import os
        os.makedirs(f'./temp_mask_for_train/folder_{self.fold}/'+ mode, exist_ok=True)
        output_name = f'./temp_mask_for_train/folder_{self.fold}/'+ mode + "/" + name + ".png"
    ...
    def __getitem__(self, idx):
        ...
        # for sim_idx in self.iou_dict[str(idx)]:
            # sim_idx = int(sim_idx)
        if self.cluster:
            query_name, _, _ = self.img_metadata_val[idx]
        else:
            query_name, _ = self.img_metadata_val[idx]
        print(query_name)
        query_name_ = []
        support_name_ = []
        input_image_ = []
        input_target_final_ = []
        prompt_image_ = []
        prompt_target_final_ = []
        len_sim = len(self.images_top50_val[query_name]['top50'])
        assert len_sim==50
        for sim_idx in range(len_sim): # 第二个evaluation需要改成1 default
            query_name, support_name, class_sample_query, class_sample_support = self.sample_episode(idx,sim_idx)
            # query_name, support_name, class_sample_query, class_sample_support = self.sample_episode_random(idx,sim_idx)
            ...
            prompt_target_final = self.save_mask(support_mask, support_name, 'support')
            input_target_final = self.save_mask(query_mask, query_name, 'query')
            

            input_image, input_target, prompt_image, prompt_target = self.load_path(query_name, support_name)
            
            query_name_.append(query_name)
            support_name_.append(support_name)
            input_image_.append(input_image)
            input_target_final_.append(input_target_final)
            prompt_image_.append(prompt_image)
            prompt_target_final_.append(prompt_target_final)

        batch = {
                'query_name': query_name_,
                'support_name': support_name_,
                'input_image': input_image_,
                'input_target': input_target_final_,
                'prompt_image': prompt_image_,
                'prompt_target': prompt_target_final_}
        #     query_img, query_cmask, support_img, support_cmask, org_qry_imsize = self.load_frame(query_name, support_name)
    ...
    def load_path(self, query_name, support_name):
        # import pdb;pdb.set_trace()
        query_img = self.read_path(query_name)
        query_mask = self.read_mask_path(query_name)
        support_img = self.read_path(support_name)
        support_mask = self.read_mask_path(support_name)
    
        return query_img, query_mask, support_img, support_mask
    
    def read_path(self, img_name):
        r"""Return image path"""
        return os.path.join(self.img_path, img_name) + '.jpg'

    def read_mask_path(self, img_name):
        r"""Return mask path"""
        return os.path.join(self.ann_path, img_name) + '.png'
    ...
    def sample_episode_random(self, idx, sim_idx):
        """Returns the index of the query, support and class."""
        if self.cluster:
            query_name, class_sample, cluster_sample = self.img_metadata_val[idx]
        else:
        ...
        # 替换sim_idx为随机
        sim_idx = np.random.randint(0, len(self.images_top50_val[query_name]['top50']))
        # print(len(self.images_top50_val[query_name]['top50']))
        support_name = self.images_top50_val[query_name]['top50'][sim_idx]
        support_class = self.images_top50_trn[support_name]['class']
