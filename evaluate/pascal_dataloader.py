...
# def create_grid_from_images_old(canvas, support_img, support_mask, query_img, query_mask, voting=False):
#     # if not voting:
#     #     canvas[:, :support_img.shape[1], :support_img.shape[2]] = support_img
#     #     canvas[:, -query_img.shape[1]:, :query_img.shape[2]] = query_img
#     #     canvas[:, :support_img.shape[1], -support_img.shape[2]:] = support_mask
#     #     canvas[:, -query_img.shape[1]:, -support_img.shape[2]:] = query_mask
#     # else:
#     ori_data = [support_img, support_mask, query_img, query_mask]
#     voting_dict = {0:[0,1,2,3], 1:[1, 0, 3, 2], 2:[3, 2, 1, 0], 3:[2, 3, 0, 1], 4:[1, 3, 0, 2], 5:[3, 1, 2, 0], 6:[2, 0, 1, 3], 7:[0, 2, 1, 3]}
#     canvas_list = []
    
#     for i in range(4):
#         new_canvas = deepcopy(canvas)

#         canvas_ = [ori_data[j] for j in voting_dict[i]]
#         new_canvas[:, :support_img.shape[1], :support_img.shape[2]] = canvas_[0]
#         new_canvas[:, -query_img.shape[1]:, :query_img.shape[2]] = canvas_[2]
#         new_canvas[:, :support_img.shape[1], -support_img.shape[2]:] = canvas_[1]
#         new_canvas[:, -query_img.shape[1]:, -support_img.shape[2]:] = canvas_[3]
#         canvas_list.append(new_canvas)
#     canvas = torch.stack(canvas_list, 0)
#     return canvas
...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, split, image_transform, mask_transform, padding: bool = 1, use_original_imgsize: bool = False, flipped_order: bool = False,
                reverse_support_and_query: bool = False, random: bool = False, ensemble: bool = False, annotation_file=None, purple: bool = False, cluster: bool = False, feature_name: str='features_vit_dino_val', percentage: str='', seed: int=0):
        self.fold = fold
        self.split = split
        self.annotaion_file = annotation_file
        self.nfolds = 4
    ...
    def get_top50_images_val(self):
        # path = "./dataset/VOC2012/features_vit-laion2b_val/folder3_top50-similarity.json"
        # path = "./20250323_only_rd20_test_bsz32_ori_filter_result_sourcefold3_fold3_rank5_10_dinov2_hodge_1_hodge_repro_2e-4_8bs.json"
        # path = f"./dataset/VOC2012/{self.feature_name}/folder{self.fold}_top50-similarity.json"
        # path = './1_20250323_only_rd20_test_bsz32_ori_filter_result_sourcefold0_fold0_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'
        # path = './wwx_1_filter_result_sourcefold0_fold0_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'
        # path = './20250410_ori_filter_result_sourcefold0_fold0_rank5_10_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'
        # path = './20250410_ori_filter_result_sourcefold{}_fold{}_rank5_10_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'.format(self.fold, self.fold)
        # path = './20250506_sourcefold{}_fold{}_rank5_10_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'.format(self.fold, self.fold)
        path = self.annotaion_file
        print(path)
        # path = './rm_cp_ori_filter_result_sourcefold0_fold0_rank5_10_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'
        # print(1)
        # with open(f"./save/bak_json/cp_test_bsz256_ori_filter_result_sourcefold3_fold3_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json") as f:
        # with open(f"./save/0.9_cp_speed_result_sourcefold3_fold3_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json") as f:
        # with open(f"./save/v0/rd20_test_bsz32_ori_filter_result_sourcefold3_fold3_rank5_dinov2_hodge_10_hodge_repro_2e-4_8bs.json") as f:
        # with open(f"./noremove_only_rd20_test_bsz32_ori_filter_result_sourcefold3_fold3_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json") as f:
        with open(path) as f:
        # with open(f"./save/v0/fold3/valsubsubonly_rd20_test_bsz32_ori_filter_result_sourcefold3_fold3_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json") as f:
            
        # with open(f"./cp_0.8_rd20_test_bsz256_ori_filter_result_sourcefold3_fold3_rank5_dinov2_hodge_1_hodge_repro_2e-4_8bs.json") as f:
    ...
    def __getitem__(self, idx):
        ...
        # for sim_idx in self.iou_dict[str(idx)]:
            # sim_idx = int(sim_idx)
        for sim_idx in range(1): # 第二个evaluation需要改成1 default
