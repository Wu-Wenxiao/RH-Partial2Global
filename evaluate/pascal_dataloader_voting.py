...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, split, image_transform, mask_transform, padding: bool = 1, use_original_imgsize: bool = False, flipped_order: bool = False,
                ...
                annotation_file=None, voting=False):
    ...
    def __len__(self):
        if '_val' in self.feature_name:
            return 1000 
        else:
            return len(self.img_metadata_val)
 
    # def get_top50_images_val(self):
    #     with open(f"/apdcephfs_cq3/share_3208175/private/chengmingxu/Datasets/VOC2012/{self.feature_name}/folder{self.fold}_top50-similarity.json") as f:
    #         images_top50 = json.load(f)

    #     # with open(f"/apdcephfs_cq3/share_3208175/private/chengmingxu/Datasets/VOC2012/{self.feature_name}/folder{self.fold}_top100-similarity.json") as f:
    #     #     images_top50 = json.load(f)
    ...
    def get_top50_images_val(self):
        ...
        # # for k, v in images_top50.items():
        # #     images_top50_new[k] = {'top50': v}
        ...
        # for line in images_top50:
        #     img_name, train_image_name = line['query'], line['gallery']
        #     images_top50_new[img_name] = {'top50': [train_image_name]}

        path = './testtime_sourcefold{}_fold{}_rank5_10_dinov2_hodge_1_hodge_repro_2e-4_8bs.json'.format(self.fold, self.fold)
    ...
    def create_grid_from_images(self, support_img, support_mask, query_img, query_mask, flip: bool = False):
        ...
        ori_data = [support_img, np.repeat(support_mask, 3, axis=0), query_img, np.repeat(query_mask, 3, axis=0)]
        voting_dict = {0:[0,1,2,3], 1:[1, 0, 3, 2], 2:[3, 2, 1, 0], 3:[2, 3, 0, 1], 4:[1, 3, 0, 2], 5:[3, 1, 2, 0], 6:[2, 0, 1, 3], 7:[0, 2, 1, 3]}
        if not self.voting:
            # canvas = [np.concatenate([ori_data[0], ori_data[1]], 1), np.concatenate([ori_data[2], ori_data[3]], 1)]
            # canvas = np.concatenate([canvas[0], canvas[1]], 2)
            canvas[:, :support_img.shape[1], :support_img.shape[2]] = ori_data[0]
            canvas[:, -query_img.shape[1]:, :query_img.shape[2]] = ori_data[2]
            canvas[:, :support_img.shape[1], -support_img.shape[2]:] = ori_data[1]
            canvas[:, -query_img.shape[1]:, -support_img.shape[2]:] = ori_data[3]
        else:
            canvas_list = []
            for i in range(8):
                canvas = torch.ones((support_img.shape[0], 2 * support_img.shape[1] + 2 * self.padding, 2 * support_img.shape[2] + 2 * self.padding))
                canvas_ = [ori_data[j] for j in voting_dict[i]]
                canvas[:, :support_img.shape[1], :support_img.shape[2]] = canvas_[0]
                canvas[:, -query_img.shape[1]:, :query_img.shape[2]] = canvas_[2]
                canvas[:, :support_img.shape[1], -support_img.shape[2]:] = canvas_[1]
                canvas[:, -query_img.shape[1]:, -support_img.shape[2]:] = canvas_[3]
    ...
    def __getitem__(self, idx):
        ...
        # total_idx = 1 if '_val' in self.feature_name else 50
        total_idx = 1
        support_names = []
        for sim_idx in range(total_idx):
            # import pdb;pdb.set_trace()
    ...
    def sample_episode(self, idx, sim_idx):
        ...
        try:
            support_name = self.images_top50_val[query_name]['top50'][0][sim_idx]
