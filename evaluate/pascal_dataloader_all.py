...
class DatasetPASCAL(Dataset):
    ...
    def __len__(self):
        # return 10
        # print(len(self.img_metadata_val))
        return len(self.img_metadata_val)
 
    def get_top50_images_val(self):

        # with open(f"./dataset/VOC2012/{self.feature_name}/folder{self.fold}_top50-simislarity.json") as f:
    ...
    def get_grid(self, query_name, support_name):
        # if self.cluster:
        ...
        class_all = []
        for name, class_name in self.img_metadata_val:
            if name == query_name:
                class_all.append(class_name)
                print(class_name)
        ...
        class_sample_query = class_all[-1]
        class_sample_support = support_class
        # return query_name, support_name, class_all[0], support_class
    ...
    def get_grids(self, query_name, support_name_all):

        grid_stack = torch.tensor([]).cuda()
        for support_name in support_name_all:
            # support_name = support_name_all[i]
            class_all = []
            for name, class_name in self.img_metadata_val:
                if name == query_name:
                    class_all.append(class_name)
    ...
    def __getitem__(self, idx):
        ...
        for sim_idx in trange(len_sim): # 第二个evaluation需要改成1 default
            # print(sim_idx)
            # time_ = time.time()
        ...
        grid_stack = torch.cat(grid_list)
    ...
    def extract_ignore_idx(self, mask, class_id, purple):
        ...
        if not purple:
            ...
            mask = np.where(mask == class_id +1, 255, 0).astype(np.uint8)
