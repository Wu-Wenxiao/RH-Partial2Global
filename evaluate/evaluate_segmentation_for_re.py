...
def evaluate(args):
    ...
    ds = {
        ...
        'pascal_det': CanvasDataset
                         ...
                         flipped_order=args.flip, purple=args.purple, annotation_file=args.annotation_file, random=args.random, cluster=args.cluster, feature_name=args.feature_name, percentage=args.percentage, seed=args.seed)
    ...
    for idx in trange(len(ds)):
        ...
        for grid_idx, canvas in enumerate(canvas_stack):
            ...
            if args.purple:
                generated_result = round_image(np.int32(generated_result), [YELLOW, PURPLE], t=args.t)
            else:
                
                generated_result = round_image(np.int32(generated_result), [WHITE, BLACK], t=args.t)
    ...
    trial_num = args.output_dir.split('/')[-1].split("_")[-5][5:]
    alpha = args.output_dir.split('/')[-1].split("_")[-6][5:]
    with open("./log_for_re/test_results_rh.txt", "a") as f:
        f.write(f'Fold-{args.fold}\t alpha-{alpha}\t trial-{trial_num}\t {str(eval_dict)}\n')
...
if __name__ == '__main__':
    ...
    #         print(f'{args.annotation_file}存在')
    # except FileNotFoundError:
    #     print(f'{args.annotation_file}不存在')
    #     sys.exit(0)
    if args.output_dir:
